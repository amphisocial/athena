...
(() => {
  ...
  const Audio = { room: null, enabled: false, url: null, on: false, checked: false, canPublish: false };
  const LK = () => window.LivekitClient;
  // The LiveKit script loads async (so it can never block the board). If the
  // teacher clicks Audio before it finished, wait briefly for it.
  ...
  async function checkAudioConfig() {
    if (NOLOGIN) { Audio.checked = true; return; } // guests/public: no audio
  ...
  // Probe that the LiveKit host is actually reachable over HTTPS before we try
  // the WebSocket, so "Failed to fetch" becomes an actionable message instead
  // of a cryptic signal-connection error. no-cors: a reachable host resolves
  // (opaque response); only a real network/DNS/TLS failure rejects.
  async function preflightAudio(wsUrl) {
    ...
    try { await fetch(httpUrl, { mode: 'no-cors', cache: 'no-store', signal: ctrl.signal }); }
      ...
      throw new Error(`can't reach the audio server at ${host}. Make sure the LiveKit server is running and reachable over HTTPS (DNS + TLS + nginx proxy for that host).`);
  ...
  async function connectAudioRoom(label) {
    ...
    let d;
    try {
      d = await api('/api/live/token', { method: 'POST', body: JSON.stringify({ kind: 'board', id: boardIdValue, label: label || '' }) });
    } catch (e) { throw new Error(e.message || 'could not get an audio token.'); }

    const url = normalizeWsUrl(d.url);
    ...
    try {
      await room.connect(url, d.token);
    } catch (e) {
    ...
    Audio.room = room; Audio.on = true; Audio.canPublish = !!d.canPublish;
    return room;
  }

  async function startTeacherAudio() {
    const btn = $('#audioToggleBtn');
    if (btn) { btn.disabled = true; btn.textContent = '… connecting'; }
    try {
      const room = await connectAudioRoom('Teacher');
      await room.localParticipant.setMicrophoneEnabled(true);
      send({ type: 'audio', on: true }); // tell viewers to start listening
      setStatus('Audio on — viewers can hear you.', 'success');
    } catch (e) {
      await stopAudio();
      setStatus('Audio: ' + e.message, 'error');
    } finally {
  ...
  function connect() {
    ...
    ws.addEventListener('message', (event) => {
      ...
      if (m.type === 'audio') {
        // Teacher started/stopped broadcasting. Viewers follow; the teacher
        // ignores this echo (they drive their own mic locally).
        if (!isOwner) { if (m.on) joinViewerAudio(); else stopAudio(); }
        return;
      }
      if (m.type === 'presence') {
