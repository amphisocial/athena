...
function attachBoardRoutes(app, deps) {
  ...
  const ANALYZE_INSTRUCTIONS = [
    ...
    '  "integral": null OR { "integrand": "the function of x being integrated, e.g. \\"x\\" or \\"x^2+1\\"", "from": lower limit as a number or null if indefinite, "to": upper limit as a number or null, "value": the evaluated result as a number or null } - fill this whenever the board shows an integral, so the area under the curve can be shaded and drawn,',
    ...
    '}',
    'Guidance by kind:',
    '- algebra/arithmetic/calculus/system: fill "steps" with a full worked solution, each with a justification. For calculus set "method". For an integral, ALSO fill "integral" with the integrand and its limits (and the value if definite) so the area under the curve can be drawn and shaded. For systems state which method and why, AND always populate "plots" with every equation solved for y so the lines/curves can be graphed together and their intersection shown.',
